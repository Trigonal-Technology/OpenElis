...
public class TestDAOImpl extends BaseDAOImpl implements TestDAO {
	...
	public List getPageOfTests(int startingRecNo) throws LIMSRuntimeException {
		...
		try {
			...
			// bugzilla 1399
			String sql = "select t from Test t left join t.testSection ts order by ts.testSectionName, t.testName";
			Query query = HibernateUtil.getSession().createQuery(sql);
			query.setFirstResult(Math.max(0, startingRecNo - 1));
	...
	public List getPageOfSearchedTests(int startingRecNo, String searchString)
		...
		try {
			...
			LogEvent.logInfo("TestDAOImpl", "getPageOfSearchedTests",
					"Starting rec: " + startingRecNo + ", searchString: [" + searchString + "]");

			// Blank search handles as browse
			if (searchString.isEmpty()) {
				LogEvent.logInfo("TestDAOImpl", "getPageOfSearchedTests",
						"Blank search string, redirecting to getPageOfTests");
				return getPageOfTests(startingRecNo);
			}
			...
			if (wCdPosition == -1) // no wild card, default to contains search
			{
				...
				sql = "select t from Test t left join t.testSection ts where (trim(lower(t.description)) like :param or trim(lower(t.testName)) like :param) order by ts.testSectionName, t.testName";
			} else {
	...
	public List getPageOfTestsBySysUserId(int startingRecNo, int sysUserId) throws LIMSRuntimeException {
		...
		try {
			...
			if (!(sectionIdList.equals("")) && (sectionIdList.length() > 0)) {
				sectionIdList = sectionIdList.substring(0, sectionIdList.length() - 1);
				sql = "select t from Test t left join t.testSection ts where ts.id in (" + sectionIdList
						+ ") order by ts.testSectionName, t.testName";
			} else {
				return list;
			}
	...
	public List getPageOfSearchedTestsBySysUserId(int startingRecNo, int sysUserId, String searchString)
		...
		try {
			...
			if (!(sectionIdList.equals("")) && (sectionIdList.length() > 0)) {
				...
				LogEvent.logInfo("TestDAOImpl", "getPageOfSearchedTestsBySysUserId", "Starting rec: " + startingRecNo
						+ ", sysUserId: " + sysUserId + ", searchString: [" + searchString + "]");

				if (searchString.isEmpty()) {
					LogEvent.logInfo("TestDAOImpl", "getPageOfSearchedTestsBySysUserId",
							"Blank search string, redirecting to getPageOfTestsBySysUserId");
					return getPageOfTestsBySysUserId(startingRecNo, sysUserId);
				}
				...
				if (wCdPosition == -1) // no wild card, default to contains search
				{
					newSearchStr = "%" + searchString.toLowerCase().trim() + "%";
					sql = "select t from Test t left join t.testSection ts where ts.id in (" + sectionIdList
							+ " ) and (trim(lower(t.description)) like :param or trim(lower(t.testName)) like :param) order by ts.testSectionName, t.testName";
				} else {
					newSearchStr = searchString.replace(wildCard, "%").toLowerCase().trim();
					sql = "select t from Test t left join t.testSection ts where ts.id in (" + sectionIdList
							+ ") and (trim(lower(t.description)) like :param or trim(lower(t.testName)) like :param) order by ts.testSectionName, t.testName";
			...
			query.setParameter("param", newSearchStr);
			LogEvent.logInfo("TestDAOImpl", "getPageOfSearchedTestsBySysUserId",
					"Executing HQL: [" + sql + "] with param: [" + newSearchStr + "]");
			query.setFirstResult(Math.max(0, startingRecNo - 1));
			query.setMaxResults(endingRecNo - 1);
	...
	public Integer getTotalSearchedTestCount(String searchString)
		...
		try {
			...
			LogEvent.logInfo("TestDAOImpl", "getTotalSearchedTestCount", "searchString: [" + searchString + "]");
			...
			{
				newSearchStr = "%" + searchString.toLowerCase().trim() + "%";
				sql = "select count (*) from Test t where (trim(lower(t.description)) like :param or trim(lower(t.testName)) like :param)";
			} else {
				newSearchStr = searchString.replace(wildCard, "%").toLowerCase().trim();
				sql = "select count (*) from Test t where (trim(lower(t.description)) like :param or trim(lower(t.testName)) like :param)";
			}
			...
			LogEvent.logInfo("TestDAOImpl", "getTotalSearchedTestCount",
					"Executing HQL Count: [" + sql + "] with param: [" + newSearchStr + "]");
			...
			if (results != null && results.get(0) != null) {
				if (results.get(0) != null) {
					...
					LogEvent.logInfo("TestDAOImpl", "CountResult", "Count is: " + count);
				}
			}
	...
	public Integer getTotalSearchedTestCountBySysUserId(int sysUserId, String searchString)
		...
		try {
			...
			if (!(sectionIdList.equals("")) && (sectionIdList.length() > 0)) {
				...
				LogEvent.logInfo("TestDAOImpl", "getTotalSearchedTestCountBySysUserId",
						"sysUserId: " + sysUserId + ", searchString: [" + searchString + "]");

				if (searchString.isEmpty()) {
					// We need getTotalSearchTestCountBySysUserId but it doesn't exist as a simple
					// count without string? No, it's just getTotalTestCount with filtering.
					// For simplicity if blank, return total count for that user (needs
					// implementation or call existing)
					// Existing getTotalTestCount doesn't take user id.
					// Let's just continue with empty string match for now or just return 0 if
					// that's what blank means in count.
					// Actually, if we want browse, we should return all.
				}
				...
				if (wCdPosition == -1) // no wild card, default to contains search
				{
					...
					sql = "select count (*) from Test t where t.testSection.id in (" + sectionIdList
							+ ") and (trim(lower(t.description)) like :param or trim(lower(t.testName)) like :param)";
				} else {
					newSearchStr = searchString.replace(wildCard, "%").toLowerCase().trim();
					sql = "select count (*) from Test t where t.testSection.id in (" + sectionIdList
							+ ") and (trim(lower(t.description)) like :param or trim(lower(t.testName)) like :param)";
	...
	public String getReferenceInfoForTestName(String name) {
		...
		loadMapsSynchronized(ID_REFERENCE_INFO_MAP);
		return ID_REFERENCE_INFO_MAP != null ? ID_REFERENCE_INFO_MAP.get(activeTestByName.getId())
				: activeTestByName.getId();
	}
	...
	public List<Test> getAllOrderBy(String columnName) throws LIMSRuntimeException {
		...
		try {
			...
			// I didn't manage to get a query parameter to be used as a column name to sort
			// by (because ORDER BY "my_column" is not valid SQL).
			...
			try {
				...
				for (Object[] objects : list) {
					...
					if (tests.get(testId) != null) {
						...
						NonNumericTests nonNumericTests = new NonNumericTests(testId, testResultType,
								new IdValuePair(testResultValue, dictEntry));
